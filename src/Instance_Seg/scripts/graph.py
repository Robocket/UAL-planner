...
class InstanceGraph(Node):
    def __init__(self):
        ...
        if subscriber_qos_depth <= 0 or log_period_sec <= 0.0:
            raise ValueError('QoS 深度和日志周期必须大于 0')
        topic = self.get_parameter('info_topic').value

        self.subscriber = self.create_subscription(
            ProjectedInstanceInfo, topic, self.instance_callback,
            subscriber_qos_depth,
        )
        self.log_timer = self.create_timer(log_period_sec, self.log_status)
